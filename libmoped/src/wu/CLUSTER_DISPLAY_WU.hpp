...
namespace MopedNS {
	...
	class CLUSTER_DISPLAY_WU:public MopedAlg {

		int display;
	public:

		CLUSTER_DISPLAY_WU( int display )
		: display(display) { }
		...
		void process(FrameData &frameData) {
			...
			for( int i = 0; i < (int)frameData.images.size(); i ++) {
				...
				for(int model=0; model<(int)clusters.size(); model++ ) {
					int objectHash = 0;
					for ( unsigned int x = 0 ; x < (*models)[model]->name.size(); x ++ ) 
					...
					foreach( cluster, clusters[model] ) {
						if( cluster.size() &&  matches[model][ cluster.front() ].imageIdx == i ) {
							foreach( match, cluster ) {
								cvCircle( img, cvPoint(matches[model][match].coord2D[0],matches[model][match].coord2D[1]), 3, color, 3 );
							}
						}
					}
				}
